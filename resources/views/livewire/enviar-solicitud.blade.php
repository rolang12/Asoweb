<div>
    <button wire:click="enviarSolicitud({{$iduser}})" class="bg-cyan-900 border-2 transition ease-in hover:-translate-y-1
     ...
     {{$status == 'Solicitud Enviada' || $status == 'Solicitud Cancelada' ? 'bg-gray-400 border-gray-300' : 'bg-cyan-900'}} rounded-2xl text-white p-3 hover:text-cyan-900 hover:bg-white">{{$status}}</button>
</div>
